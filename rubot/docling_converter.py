...
class DoclingPDFConverter:
    ...
    def __init__(self, config: DoclingConfig):
        ...
        self.logger.info(f"Initializing Docling converter with OCR engine: {config.ocr_engine}")
        self.logger.info(f"OCR enabled: {config.do_ocr}, Table structure: {config.do_table_structure}")
    ...
    def convert_to_markdown(self, pdf_path: str) -> str:
        ...
        try:
            ...
            image_mode = image_mode_map.get(self.config.image_mode, ImageRefMode.PLACEHOLDER)
